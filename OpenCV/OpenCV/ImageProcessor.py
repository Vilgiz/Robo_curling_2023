...
class ImageProcessor():
    ...
    def __get_rotaion_matrix(self, markers):
        ...
        marker2 = markers[2]
        marker3 = markers[3]
        self.rotation_angle = -math.atan((marker2.center[0] - marker1.center[0])/(marker2.center[1] - marker1.center[1]))
    ...
    def aruco_calibration(self, image):
        ...
        if type(markers) is not None:
            self.__get_rotaion_matrix(markers)
            for marker in markers:
                marker = markers[marker]
                cv2.circle(image, marker.center, 4, (0, 0, 255), -1)

    def warp(self, image):
        cos_rm = np.abs(self.rotation_matrix[0][0])
        sin_rm = np.abs(self.rotation_matrix[0][1])    
        new_height = int((image.shape[0] * sin_rm) + (image.shape[1] * cos_rm))
        new_width = int((image.shape[0] * cos_rm) +  (image.shape[1] * sin_rm))
        ...
        cv2.namedWindow('h', flags=cv2.WINDOW_KEEPRATIO)
