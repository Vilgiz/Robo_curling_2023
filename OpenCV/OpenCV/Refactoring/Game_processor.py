    ...
    def __geometry_init(self):
        ...
        for k in [0, 1]:
            ...
            for j in range(self.num_sectors):
                sector_center_x = int(self.center[0] + r * np.cos(math.radians(self.sector_size*j + self.sector_size/2)))
                sector_center_y = int(self.center[1] + r * np.sin(math.radians(self.sector_size*j + self.sector_size/2)))
...
class Brain():
    ...
    def __field_scanning(self, data):
        for i in range(len(data)):
            distance = math.sqrt((data[i][1]-self.Field.center[0])**2 + (data[i][2]-self.Field.center[1])**2)
            angle = math.degrees(math.atan2(data[i][2]-self.Field.center[1], data[i][1]-self.Field.center[0]))
    ...
    def __near_field_check(self, radius, point_idx):
        ...
        for i in range(len(self.data)):
            if (self.data[i][0] != 0) and (i != point_idx):
                distance = math.sqrt((self.data[i][1] - point[1])**2 + (self.data[i][2] - point[2])**2)
                if distance <= radius:
                    if self.data[i][0] == 1:
                        count_human += 1
        ...
        return [count_robot, count_human]
        ...
        for i in range(len(data)):
            if (data[i][0] == 1) and (data[i][3] == 3):
                ...
                if self.embedded_sectors[data[i][3]-1][data[i][4]] == 1:
                    ...
                    if surround [1] > 1:
                        coef = surround [0] - surround [1]
                        solution_matrix.append([self.fast_priority[data[i][3]-1][data[i][4]] + coef,'destroy',[data[i][1], data[i][2]]])
                    else:
                        solution_matrix.append([self.fast_priority[data[i][3]-1][data[i][4]] ,'fast',[data[i][1], data[i][2]]])
            ...
            if (data[i][0] != 0) and (data[i][3] == 2):
                for j in range(5):
                    ...
                    free_sectors [1][k] = 0
                    free_sectors [1][l] = 0
        normal_priority = [[x*y for x, y in zip(row_a, row_b)] for row_a, row_b in zip(self.normal_priority, free_sectors)]
        for i in range(2):
            for j in range(len (normal_priority[0])):
                if (normal_priority[i][j] !=0 ):
                    solution_matrix.append([normal_priority[i][j], 'normal', self.Field.centers[i][j]])
    ...
    def draw_plt(self):
        ...
        for i in range(4):
            circle = plt.Circle(self.Field.center, self.Field.radius[i], color='blue' if i < 2 else 'red', fill=False)
            ax.add_artist(circle)
        for j in range(self.Field.num_sectors):
            sector = patches.Wedge(self.Field.center, self.Field.radius[0], angles[j], angles[j+1], fill=False, color='black', lw=0.5)
        ...
        if self.result[2] == ['destroy']:color = 'red'
        elif (self.result[2] == ['fast']):color = 'yellow'
        else:color = 'green'
        plt.plot ([self.result[0][0],self.result[1][0]],[self.result[0][1],self.result[1][1]], color = color)
        plt.axis('scaled')
        plt.show()
